...
export class AnalyticsController {
  ...
  @ApiOperation({ summary: 'Cache statistics', description: 'Hit rate, miss count, and estimated cost savings from the response cache' })
  @ApiResponse({ status: 200, description: 'Cache stats for the authenticated tenant' })
  @Get('cache')
  async getCacheStats(
  ...
  @ApiOperation({ summary: 'Usage time series', description: 'Token usage and request counts grouped by day or hour' })
  @ApiQuery({ name: 'start', description: 'Start date (ISO 8601)', example: '2026-03-01' })
  @ApiQuery({ name: 'end', description: 'End date (ISO 8601)', example: '2026-03-31' })
  @ApiQuery({ name: 'granularity', required: false, enum: ['day', 'hour'], example: 'day' })
  ...
  @ApiOperation({ summary: 'Request log', description: 'Paginated log of all gateway requests with filters' })
  @ApiQuery({ name: 'page', required: false, example: 1 })
  @ApiQuery({ name: 'limit', required: false, example: 50 })
  @ApiQuery({ name: 'provider', required: false, example: 'anthropic' })
  @ApiQuery({ name: 'status', required: false, enum: ['success', 'error', 'cached', 'rate_limited'] })
  ...
  @ApiOperation({ summary: 'Cost breakdown', description: 'Total spend and per-provider cost breakdown for a date range' })
  @ApiQuery({ name: 'start', description: 'Start date (ISO 8601)', example: '2026-03-01' })
  @ApiQuery({ name: 'end', description: 'End date (ISO 8601)', example: '2026-03-31' })
